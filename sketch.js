...
function draw() {
  ...
  if (gameState === PLAY) {
    ...
    if (monkey.isTouching(bananaGroup)) {
      ...
      else monkey.scale = map(score, 0, 50, 0.18, 0.18);
    }
  ...
  if (gameState === LOST) {
    textSize(30);
    fill(255);
    text("Game Over", camera.position.x - 80, 200);
  ...
  fill("white");
  textSize(24);
  text("Press right arrow to move faster", monkey.x- 150, 100);
  //displaying score
  textSize(40);
...
function spawnStone() {
  if (frameCount % 150 === 0) {
    stone = createSprite(monkey.x + 200, 450);
